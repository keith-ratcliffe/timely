...
echo "$JAVA_HOME/bin/java ${JVM_ARGS} qonduit.StandaloneServer "${TMP_DIR}" --spring.config.name=qonduit --spring.profiles.active=standalone"
$JAVA_HOME/bin/java ${JVM_ARGS} qonduit.StandaloneServer "${TMP_DIR}" --spring.config.name=qonduit --spring.profiles.active=standalone
